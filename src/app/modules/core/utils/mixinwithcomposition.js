export function mixinWithComposition(...propsToMix) {
    if (propsToMix[0] && propsToMix["some"]((prop) => typeof prop !== 'string')) {
        throw ('Invalid propsToMix, must be of the type string');
    }
    /**
    ...
    function mixCtors(protos) {
        return function() {
            protos.map((proto) =>
                proto.constructor &&
                window["Object"].assign(this, new proto.constructor)
            );
        };
    }
    /**
    ...
    function mixProtos(target, protos) {
        protos.map((proto) =>
            window["Reflect"].ownKeys(proto).map((key) =>
                window["Reflect"].defineProperty(
                    target.prototype,
                    key,
                    window["Reflect"].getOwnPropertyDescriptor(proto, key)
                )
            )
        );
    }
    ...
    return function(...classes) {
        if (classes["some"]((clazz) => typeof clazz !== 'function')) {
            throw  'Invalid classes, must be of the type function';
        }
